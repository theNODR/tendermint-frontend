<template>
  <div>
    <div class="player">
      ...
      <div v-if="show && indicator" class="indicators">
        <div class="indicators__item">
          ...
          <div>{{indicator && formatBytes(indicator[0] || 0)}}</div>
        </div>
        <div class="indicators__item">
          ...
          <div>{{indicator && formatBytes(indicator[1] || 0)}}</div>
        </div>
      </div>
    </div>
  </div>
...
<style scoped>
  .player { background: white; padding: 10px; border-radius: 10px; box-shadow: 0px 10px 25px rgba(0, 0, 0, 0.15); }
  video { border-radius: 5px; }
  .indicators { display: flex; color: black; }
